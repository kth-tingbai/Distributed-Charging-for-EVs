...
import numpy as np
import time
from Rollout_individual_vehicle import road_charge, regulation_market, EV_charge
...
f.close()
    ...
    # get the lower and upper bound of the solution, using timid, greedy, and relaxed base policy
    #up_bound1,_=ev.time_schedule(ev.timid_charging, ev.timid_overdue, road, regulation) # upper bound
    #up_bound2,_,_=ev.time_schedule(ev.greedy_charging, road, regulation) # upper bound
    #up_bound3,_,_=ev.time_schedule(ev.relaxed_charging, road, regulation) # upper bound
    
    #%% Three rollout-based solutions (RS)
    # using timid base policy
    
    #start_t1=time.perf_counter()
    #cost1, b_charge1, t_charge1, t_extra1=ev.rollout(ev.timid_charging, ev.timid_overdue, road, regulation) 
    #end_t1=time.perf_counter()
    #interval_t1= round((end_t1-start_t1)/4,4)
    
    #Solution_5[round(e_0_rate,2)].append([cost1,b_charge1,t_charge1,t_extra1,interval_t1])


    # using greedy base policy
    #start_t2=time.perf_counter()
    #cost2, b_charge2, t_charge2, t_extra2=ev.rollout_para_test(ev.greedy_charging, road, regulation) 
    #end_t2=time.perf_counter()
    #interval_t2= round((end_t2-start_t2)/4,4)
    

    # using relaxed base policy
    #start_t3=time.perf_counter()
    #cost3,b_charge3, t_charge3, t_extra3=ev.rollout_para_test(ev.relaxed_charging, road, regulation) 
    #end_t3=time.perf_counter()
    #interval_t3= round((end_t3-start_t3)/4,4)
    
    #%% The exact soluion (OS)
    #pre = time.perf_counter()
    #start_t4=time.perf_counter()
    #ev.best_exhaust(road, regulation) # optimal solution
    #end_t4=time.perf_counter()
    #nterval_t4= round((end_t4-start_t4)/4,4)

    #cost4=ev.best.copy()
    #b_charge4=ev.best_charging.copy()
    #_charge4=ev.best_t_charge.copy()
    #t_extra4=ev.best_t_extra.copy()
    
    #%% The linearization solution (LS)
    #start_t5=time.perf_counter()
    #cost5, t_charge5, t_extra5, b_charge5,b_rest5=ev.best_linearization(road, regulation)
    #end_t5=time.perf_counter()
    #interval_t5= round((end_t5-start_t5)/4,4)
