# -*- coding: utf-8 -*-
"""
Created on Fri Oct 13 16:46:28 2023
...
class road_charge:
    def __init__(self, tau, detour, Pcharging, wait):
...
class EV_charge:
    ...
    def __init__(self, Pmax, eusable, eini, Pbar):
    ...
    def lower_bound(self, road_charge, regulation_market):
        ...
        for i in range(N):
            ...
            constr += [delta_T[i+1]==delta_T[i]+Delta_ehat[i]/Pactual[i]+b_charging[i]*wait[i]]
            cost += Delta_ehat[i]/Pactual[i]+b_charging[i]*wait[i]+b_charging[i]*2*detour[i]+Delta_ehat[i]/Pactual[i]*price
    ...
    def time_schedule(self, b_charging, b_overdue, road_charge, regulation_market):
        ...
        if detour.dot(b_charging) <= (Tbard-tau.sum())/2:
            ...
            for i in range(N):
                ...
                constr += [t_charging[i] >= 0]
                constr += [t_charging[i] <= b_charging[i]*Tbound]
                constr += [delta_T[i+1]==delta_T[i]+b_charging[i]*(t_charging[i]+wait[i])]
                cost += b_charging[i]*(t_charging[i]+wait[i]+2*detour[i])+b_charging[i]*t_charging[i]*price
        ...
        return op_value, op_t_charge # upper bound
    #------------------------------------------------------------------------------
    ...
    def rollout(self, b_charging_ini, b_overdue_ini, road_charge, regulation_market):
        ...
        return rollout, rollout_charging, rollout_t_charge, rollout_overdue
